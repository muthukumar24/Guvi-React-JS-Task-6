...
function UserList() {
  ...
  const updateUser = async (user) =>{
    try{
      await axios.patch(`https://jsonplaceholder.typicode.com/users/${user.id}`, user);
      setUsers(users.map((u) =>(u.id === user.id ? user : u))); // Update users state with updated user
      setEditingUser(null); // Reset editing user state
  ...
  const deleteUser = async (id) => {
    try{
      ...
      setUsers(users.filter((user) => user.id !== id)); // Update users state by filtering out the deleted user
